...
screen_buffer_t * ph_screen;
lcd_display_arrangement_t * lcd_layout;
static const char *TAG = "DISPLAY";

#define ASCII_OFFSET    32u   

uint8_t ascii_decoder[256][6] = {
...
{
    ph_screen = (screen_buffer_t*)malloc(sizeof(screen_buffer_t));
    
    lcd_layout = (lcd_display_arrangement_t*)malloc(sizeof(lcd_display_arrangement_t));
    ...
    for(int i = 0; i < 504; i++)
    {
        ph_screen->screen_buffer[i] = 0x00;
    }   
    for(int j = 0; j<6; j++)
    {
        ph_screen->character_buffer[0].pixels[j] = ascii_decoder[(int)(111)][j];
    }
    for(int j = 0; j<6; j++)
    {
        ph_screen->character_buffer[1].pixels[j] = ascii_decoder[(int)(110)][j];
    }
    draw();
}
...
void draw(void)
{
    ...
    for(int i = 0; i < 504; i++)
    {
        i2c_send_data(ph_screen->screen_buffer[i]);
    }
...
void draw_area(uint8_t cs, uint8_t ce, uint8_t rs, uint8_t re)
{
    ...
    for(int i = rs; i <= re; i++)
    {
        for(int j = cs; j <= ce; j++)
        {
            for(int k = 0; k < 6; k++)
            {
                i2c_send_data(ph_screen->screen.rows[i].characters[j].pixels[k]);
            }
        }
    }
}
...
void draw_char(int row, int col, char c)
{
    for(int i = 0; i<6;i++)
    {
        ph_screen->screen.rows[row].characters[col].pixels[i] = ascii_decoder[(int)(c)][i];
    }
}


void update_item(display_layout_enum_t item_id, char *x) //"haoyang liu"
{
    for(int i = 0; i < lcd_layout->item[item_id].size; i++)
    {   
        for(int j = 0; j<6; j++)
        {
            ph_screen->character_buffer[i + lcd_layout->item[item_id].start_addr].pixels[j] = ascii_decoder[(int)(x[i])][j];
        }
    }
